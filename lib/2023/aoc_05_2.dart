...
int getSolution(List<String> file) {
  ...
  for (int i = 0; i < seedMap.length; i++) {
    print(i.toString());
    if (i.isEven || i == 0) {
      for (int j = seedMap[i]; j < seedMap[i] + seedMap[i + 1]; j++) {
        // j = seedID
        seedManual = [Seed(j, null, null, null, null, null, null, null)];
        addSoil();
        addFertilizer();
        addwater();
        addLight();
        ...
        addHumidity();
        addLocation();
        //print('seed: ${seedManual[0].id}, location= ${seedManual[0].location}');
        if (result == 0) {
          result = seedManual[0].location!;
        }
        if (seedManual[0].location! < result) {
          result = seedManual[0].location!;
        }
        //print(result);
      }
