int getSolution(List<String> file) {
  ...
  for (var trailhead in trailheads) {
    ...
    result +=
        calculateTrailheadScore(trailhead, input, mapWithReachtedEndpoints);
  }
  return result;
}

int calculateTrailheadScore(Map<String, int> locationOnMap,
    List<List<int>> input, List<List<int>> endPoints) {
  findNextStep(locationOnMap, input, endPoints, 1);
  int trailHeadRating = 0;
  for (var row in endPoints) {
    for (var number in row) {
      if (number > 0) {
        trailHeadRating += number;
      }
    }
  }
...
void findNextStep(Map<String, int> locationOnMap, List<List<int>> input,
  ...
  if (numberToFind == 9) {
    for (var possibleDirection in possibleDirections) {
      endPoints[possibleDirection['x']!][possibleDirection['y']!] += 1;
    }
  } else {
    for (var possibleDirection in possibleDirections) {
